...
module.exports = {
	...
	async execute (client, message, args) {
        ...
        if (!target.bannable) return message.reply("that member can't be banned!")
        const author = message.guild.members.cache.get(message.author.id)
        if (author.roles.highest.comparePositionTo(target.roles.highest.id) <= 0) return message.reply("you can't ban someone with a higher/equal role than you!")
